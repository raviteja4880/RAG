...
class LLMService:
    ...
    def _prepare_prompt(self, query: str, context: str, history: List = None) -> List:
        ...
        system_prompt = (
            ...
            "2. If the user asks for an 'overview', 'summary', or 'description', analyze ALL provided chunks. "
            "Even if the context seems partial, synthesize a natural overview of the available facts "
            "(mentioning key entities, project names, and themes) instead of giving up.\n"
            "4. FORM EXTRACTION: If the context looks like a government form or receipt, perform fuzzy matching. "
            "For example, 'Given Name' and 'Surname' combined satisfy a request for 'Applicant Name'. "
            "Do not be overly literal; extract the logical answer from key-value triplets.\n"
            "5. Maintain a professional, concise, and technical tone. Never mention context or tool names.\n"
            "6. NO HALLUCINATION. If a specific detail (like a date) isn't there, don't invent it, but do describe the general subject.\n"
            "6. AMBIGUITY GUARDRAIL: If the context contains information from multiple different documents "
            "and you are unsure which one the user is asking about, ASK THE USER FOR CLARIFICATION "
